...
if (!fs.existsSync('tokens.txt')) {
  console.log(chalk.red('❌ File tokens.txt tidak ditemukan! Silakan buat file dengan token JWT. ⚠️'));
  process.exit(1);
}

const tokens = fs.readFileSync('tokens.txt', 'utf-8').trim().split('\n').filter(Boolean);
if (tokens.length === 0) {
  console.log(chalk.red('❌ File tokens.txt kosong! Silakan isi dengan token JWT. ⚠️'));
  process.exit(1);
}
...
if (botOption === '1') {
  ...
  // Mining
  console.log(chalk.cyan(`Starting mining with ${tokens.length} tokens 🔑`)); // Tampilkan jumlah token sebelum mining
  for (const token of tokens) {
    const proxy = useProxy === 'y' && proxies.length > 0 ? proxies[proxyIndex % proxies.length] : null;
    const agent = proxy ? (proxy.startsWith('socks') ? new SocksProxyAgent(proxy) : new HttpProxyAgent(proxy)) : null;
